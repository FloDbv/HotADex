...
// --- BRIDGE: structures unified dataset (legacy adapter for old code) ---
window.structuresData = window.structuresData || [];

// Convert a unified item into the old creatureBanksData shape
function toLegacyBank(item) {
  const legacy = {
    name: item.name,
    guards: item.guards || "",
    rewards: { creatures: null, artifacts: null, gold: null, resources: null, experience: null },
    levels: null,
    terrain: item.terrain || "",
    value: item.rmgValue || "",
    additional_info: item.notes || item.inDepth || ""
  };
  if (Array.isArray(item.levels) && item.levels.length) {
    legacy.levels = item.levels.map(l => ({
      level: l.level,
      guards: l.guards || "",
      rewards: l.rewards || "",
      experience: l.xp || l.experience || ""
    }));
  } else {
    if (item.rewards && typeof item.rewards === "object") {
      legacy.rewards = {
        creatures: item.rewards.creatures ?? null,
        artifacts: item.rewards.artifacts ?? null,
        gold: item.rewards.gold ?? null,
        resources: item.rewards.resources ?? null,
        experience: item.rewards.experience ?? null,
        text: null
      };
    } else {
      legacy.rewards = { creatures: null, artifacts: null, gold: null, resources: null, experience: null, text: (typeof item.rewards === "string" ? item.rewards : null) };
    }
  }
  return legacy;
}

// Build legacy array for any old code paths that still use creatureBanksData
window.creatureBanksData = (window.structuresData || [])
  .filter(o => Array.isArray(o.categories) && o.categories.includes("Creature Banks"))
  .map(toLegacyBank);
...
/* ================= NEW STRUCTURES RENDERER (unified dataset) ================= */
...
function ensureStructuresScaffold(){
  let view = document.getElementById("mapobjects-view") || document.getElementById("structures-view");
  if(!view){
    view = document.createElement("div");
    view.id = "structures-view";
    (document.getElementById("app") || document.body).appendChild(view);
...
function takeOverStructuresView(category="All"){
  ensureStructuresScaffold();
  renderStructuresList(category);
}
window.renderMapObjectsView = function(){ takeOverStructuresView("All"); };
window.renderStructuresView = function(){ takeOverStructuresView("All"); };
window.renderCreatureBanks = function(){ takeOverStructuresView("Creature Banks"); };

(function hookStructuresNavStrong(){
  function clickHandler(){ setTimeout(()=>takeOverStructuresView("All"), 0); }
  document.querySelectorAll("a,button,div,span,li").forEach(node=>{
    const t=(node.textContent||"").trim().toLowerCase();
    if(t==="structures") node.addEventListener("click", clickHandler);
  });
})();
document.addEventListener("DOMContentLoaded", ()=>{ setTimeout(()=>takeOverStructuresView("All"), 0); });
